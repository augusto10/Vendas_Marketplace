...
export default async function AtacadoPedidosPage({ searchParams }: { searchParams: Promise<{ start?: string; end?: string; pedido?: string }> }) {
  ...
  const canManagePedidos = hasPermission(user, "atacado.pedidos.update");
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <AtacadoPedidosTable pedidos={pedidoRows} isMaster={isMaster} canManagePedidos={canManagePedidos} />
        </CardContent>
      </Card>
    </div>
  );
