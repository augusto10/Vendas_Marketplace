        ...
        <div className="grid gap-3 content-center">
          {selected ? (
            <div className="rounded-md border border-primary/30 bg-primary/10 px-3 py-2 text-sm">
              ...
              <div className="mt-1 font-semibold text-foreground">{selected.label}</div>
          ...
          {data.map((item, index) => (
            <button
              ...
              type="button"
              onClick={() => setSelectedIndex(index)}
              className={`flex items-center justify-between gap-3 rounded-md border px-3 py-2 text-left text-sm transition-colors ${selectedIndex === index ? "border-primary/40 bg-primary/10" : "bg-muted/35 hover:bg-muted/55"}`}
            >
    ...
    <div className="grid min-h-[300px] grid-cols-[repeat(auto-fit,minmax(72px,1fr))] items-end gap-3">
      {data.map((item, index) => (
        <div key={item.label} className="flex min-h-[280px] flex-col justify-end gap-2">
          <div className="text-center text-xs font-semibold text-foreground">{item.value.toLocaleString("pt-BR")}</div>
          <div className="mx-auto flex h-52 w-full max-w-[74px] items-end rounded-t-md bg-muted/60">
            <button
              type="button"
          ...
          <div className="line-clamp-2 min-h-10 text-center text-xs text-muted-foreground" title={item.label}>
            {item.label}
          </div>
        </div>
      ))}
