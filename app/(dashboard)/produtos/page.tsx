...
import { ProductsByStatePieChart } from "@/components/charts/products-by-state-pie-chart";
import { EmptyState } from "@/components/empty-state";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Table, TableBody, TableCell, TableFooter, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { parsePeriod } from "@/lib/period";
import { getProductsByStateReport, getProductsReport } from "@/lib/services/report-service";
...
export default async function ProdutosPage({ searchParams }: { searchParams: Promise<{ start?: string; end?: string }> }) {
  ...
  const [products, productsByState] = await Promise.all([
    getProductsReport(period),
    getProductsByStateReport(period)
  ]);
  const topSalesProducts = [...products].sort((left, right) => right.rows - left.rows).slice(0, 5);
  const topProductsByState = productsByState.slice(0, 10);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            <TableHeader><TableRow><TableHead>Primeira venda</TableHead><TableHead>Ultima venda</TableHead><TableHead>SKU</TableHead><TableHead>Produto</TableHead><TableHead>Unidades</TableHead><TableHead>Receita</TableHead><TableHead>Comissao</TableHead><TableHead>Devolucoes</TableHead></TableRow></TableHeader>
            <TableBody>
              {products.map((row) => (
      ...
      <Card className="mt-8">
        <CardHeader><CardTitle>Graficos de produtos</CardTitle></CardHeader>
        <CardContent>
          <div className="grid gap-6 md:grid-cols-2">
            <div className="min-w-0">
              <div className="mb-2 text-sm font-medium">Top 5 produtos mais vendidos</div>
              {topSalesProducts.length ? (
                <TopProductsSalesChart data={topSalesProducts} className="h-80" />
              ) : (
            ...
            <div className="min-w-0">
              <div className="mb-2 text-sm font-medium">Produtos por UF</div>
              {topProductsByState.length ? (
                <ProductsByStatePieChart data={topProductsByState} className="h-80" />
              ) : (
                <EmptyState title="Sem UF no periodo" description="Importe dados com pedidos vinculados a UF para visualizar a distribuicao." />
              )}
            </div>
          </div>
        </CardContent>
