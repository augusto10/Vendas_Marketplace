...
import { carteiraDebitoSchema } from "@/lib/atacado/schemas";
import { criarDebitoManual } from "@/lib/atacado/service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = carteiraDebitoSchema.parse(await request.json());
    const movimento = await criarDebitoManual(id, {
      valor: body.valor,
